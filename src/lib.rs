//! # PDQ Hash Library
//!
//! This library provides an implementation of Facebook's PDQ (Pretty Darn Quick) perceptual hashing algorithm.
//! It also includes a SNARK-based zero-knowledge proof system for proving knowledge of an image that hashes to a specific PDQ hash.
//!
//! ## Features
//! - Generate PDQ hashes from images
//! - Compare image similarity using hamming distance
//! - Generate zero-knowledge proofs about PDQ hashes (with the `snark` feature)
//!
//! ## Example: Basic Usage
//! ```no_run
...
//!
//! // Load an image
//! let img = open("path/to/image.jpg").unwrap();
//!
//! // Generate PDQ hash
//! if let Some((hash, quality)) = generate_pdq(&img) {
//!     println!("PDQ Hash: {:02x?}", hash);
//!     println!("Quality: {}", quality);
//! }
//! ```
//!
//! ## Example: SNARK Proof (requires `snark` feature)
//! ```no_run
//! # #[cfg(feature = "snark")]
//! # {
//! use pdqhash::{generate_pdq, PDQSnark};
...
//! let (target_hash, _) = generate_pdq(&img).unwrap();
//!
//! // Generate proof
//! let (proof, public_inputs) = snark.create_proof(&image_data, target_hash, &mut rng).unwrap();
//!
//! // Verify proof
//! let is_valid = snark.verify_proof(&proof, &public_inputs).unwrap();
...
/// Add `pdqhash = { version = "*", features = ["snark"] }` to your `Cargo.toml`
/// to enable SNARK functionality.
#[cfg(feature = "snark")]
pub mod snark;

// Re-export commonly used items
pub use dwn_pdq::generate_pdq;
pub use dwn_pdq::PDQ_HASH_LENGTH;

// Re-export SNARK-related items when the feature is enabled
#[cfg(feature = "snark")]
pub use snark::PDQSnark;
...
fn to_luma_image(image: &image::DynamicImage) -> (usize, usize, Vec<f32>) {
    match image {
        ...
        _ => panic!("Unsupported DynamicImage variant for to_luma_image"),
    }
}
